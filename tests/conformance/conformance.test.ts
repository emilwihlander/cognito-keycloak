...
import { setupContainer, teardownContainer, USER_POOL_ID } from "../integration/setup.js";
...
function normalizeForCompare(result: Normalized): Normalized {
  if (!result.ok) {
    return { ok: false, error: { name: result.error.name, httpStatusCode: result.error.httpStatusCode } };
  }
...
async function cleanupTestUsers(
  ...
  try {
    ...
    console.warn(`Warning: Failed to cleanup test users from ${userPoolId}:`, err);
  }
}

describe.skipIf(!process.env.REAL_USER_POOL_ID)("Conformance tests (emulator vs real AWS Cognito)", () => {
  ...
  it("AdminCreateUser: invalid payload (missing Username) should match Cognito's validation behavior", async () => {
    // Intentionally invalid input to avoid mutating the real pool.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  it("User lifecycle (mutating): create → update attrs → set password → disable/enable → delete", async () => {
    const diffs: Array<{ step: string; local: Normalized; real: Normalized }> =
      [];
    ...
    if (diffs.length > 0) {
      throw new Error(`Conformance failure(s):\n${JSON.stringify(diffs, null, 2)}`);
    }
  });
});
