import { DescribeUserPoolRequest, DescribeUserPoolResponse, SchemaAttributeType } from "@aws-sdk/client-cognito-identity-provider";
...
const stringAttr = (name: string, opts?: { required?: boolean; mutable?: boolean; min?: string; max?: string }): SchemaAttributeType => ({
  Name: name,
  AttributeDataType: "String",
  DeveloperOnlyAttribute: false,
  Mutable: opts?.mutable ?? true,
  Required: opts?.required ?? false,
  StringAttributeConstraints: { MinLength: opts?.min ?? "0", MaxLength: opts?.max ?? "2048" },
});
...
const SCHEMA_ATTRIBUTES: SchemaAttributeType[] = [
  ...
  stringAttr("address"),
  { Name: "updated_at", AttributeDataType: "Number", DeveloperOnlyAttribute: false, Mutable: true, Required: false, NumberAttributeConstraints: { MinValue: "0" } },
  { Name: "identities", AttributeDataType: "String", DeveloperOnlyAttribute: false, Mutable: true, Required: false, StringAttributeConstraints: {} },
];

export async function describeUserPool(request: DescribeUserPoolRequest): Promise<DescribeUserPoolResponse> {
